...
> 多租户 SaaS 身份管理 — ASP.NET Core 8 后端（NSwag codegen 读 shared/openapi.yaml）。
> 与 react/vue/nextjs/springboot 同构（共享 OpenAPI.yaml 单一来源）。字段照抄 BASE（ADR-0024）；结构性对齐按 shared 9/7 重构。
...
| M00.F02.I06 | 邀请成员 | 接口 | 前端+后端 | 生成 7 天有效邀请链接并发送邮件 | 已上线 |
| M00.F02.I07 | 接受邀请 | 接口 | 前端+后端 | 凭邀请 token 完成加入；过期返 410 | 规划 |
| M00.F02.I08 | 状态切换 | 接口 | 前端+后端 | 启用/停用成员账号（不影响全局用户记录） | 已上线 |
...
| M01.F04.I03 | 密码登录 UI | 页面 | 仅前端 | 前端仓负责实现 | 已废弃 |
...
| 子项 ID | 名称 | 类型 | 交付 | 说明 | 状态 |
|---|---|---|---|---|---|
| M04.F03.I01 | 授权码签发 | 接口 | 前端+后端 | 校验 session+redirect_uri+scope 后签发一次性 authorization_code | 已上线 |
| M04.F03.I02 | 令牌交换 | 接口 | 前端+后端 | 用 authorization_code 换取 access_token + refresh_token | 已上线 |
| M04.F03.I03 | 令牌刷新 | 接口 | 前端+后端 | 用 refresh_token 换取新的 access_token（可同时轮换 refresh_token） | 已上线 |
